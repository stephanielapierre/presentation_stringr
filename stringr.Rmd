...
* La structure des fonctions et des arguments est cohérente et consistante. Le premier argument d'une fonction est toujours le vecteur sur lequel on travail. C'est d'ailleurs ce qui fait que *stringr* est particulièrement adapté pour l'utilisation des pipes (%>%)
* Simplifie les opérations
* Les résultats des fonctions de *stringr* peuvent facilement être réutiliser. Le package permet notamment de s'assurer que les données manquantes demeurent des données manquantes.
...
La fonction `str_trim` permet d'éliminer d'une chaîne de caractères tous les espaces superflus à gauche, à droite ou des deux côtés
...
---
background-image: url(https://unsplash.com/photos/hMwl23e44_Q/download?ixid=MnwxMjA3fDB8MXxzZWFyY2h8MXx8dGl0YW5pY3x8MHx8fHwxNjM3NTI0Nzkx&force=true&w=1920)
...
On a vu que les titres de civilité sont précédé d'un espace et se termine par un point.

<br /> 

L'expression régulière est composée de trois parties

1. `(?<=\\s)` indique que la chaîne de caractères qu'on recherche est précédé (?<=) d'un espace (\\s).
1. `[[:alpha:]]+` indique que la chaîne de caractères qu'on recherche est composé d'un ou plusieur (+) lettre ([[:alpha:]]).
