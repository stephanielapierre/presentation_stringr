...
# Pour mettre tous les caractères en minuscule
str_to_lower (x)
...
# Pour mettre des majuscules dans un titre
str_to_title(x)
```
...
La fonction `str_sub` pour extraire une partie des caractères. Cette fonction est composée de trois arguments :
...
La fonction `str_sub` peut aussi être utiliser pour modifier un caractère
```{r eval=TRUE}
x <- c("mercure", "vénus", "terre", "mars", "jupiter", "saturne", "uranus", "neptune", NA)
